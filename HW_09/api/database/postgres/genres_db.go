...
func (g GenresRepository) All(_ context.Context, filter *models.GenreFilter) ([]*models.Genre, error) {
	genres := make([]*models.Genre, 0)
	basicQuery := "SELECT * FROM genres"
	if filter.Query != nil {
		basicQuery = fmt.Sprintf("%s WHERE name ILIKE $1", basicQuery)
		if err := g.connection.Select(&genres, basicQuery,
			"%"+*filter.Query+"%"); err != nil {
			return nil, err
		}
		return genres, nil
	}
	if err := g.connection.Select(&genres, basicQuery); err != nil {
		return nil, err
	}
	return genres, nil
}
...
func (g GenresRepository) ByID(_ context.Context, id int) (*models.Genre, error) {
	...
	if err := g.connection.Get(genre, "SELECT * FROM anime WHERE genre_id = $1", id); err != nil {
		return nil, err
	}
